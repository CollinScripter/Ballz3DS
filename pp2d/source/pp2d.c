...
void pp2d_draw_circle(float cx, float cy, float r, u32 color, int num_segments) {
    ...
    if (vertexData.cur + (num_segments*3) > PP2D_MAX_VERTICES) {
        return;
    }
    ...
    for(int i = 0; i <= num_segments; i++) {
      pp2d_add_text_vertex(x + cx, y + cy, PP2D_DEFAULT_DEPTH, 0, 0);
      pp2d_add_text_vertex(cx, cy, PP2D_DEFAULT_DEPTH, 0, 0);
      t = x;
      x = c * x - s * y;
      y = s * t + c * y;
      pp2d_add_text_vertex(x + cx, y + cy, PP2D_DEFAULT_DEPTH, 0, 0);
...
size_t pp2d_get_verticies() {
  return vertexData.cur;
}

void pp2d_draw_square(int x, int y, int size, u32 color) {
    pp2d_draw_rectangle(x, y, size, size, color);
}
